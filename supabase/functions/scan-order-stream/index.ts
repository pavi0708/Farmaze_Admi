  ...
  if (req.headers.get('upgrade') === 'websocket') {
    ...
    socket.onmessage = async (event) => {
      try {
        ...
        if (data.type === 'scan_order' && data.imageBase64) {
          await processScanOrderStream(socket, data.imageBase64);
        }
      } catch (error) {
...
async function processScanOrderStream(socket: WebSocket, imageBase64: string) {
  try {
    ...
    socket.send(JSON.stringify({
      type: 'progress',
      stage: 'credentials',
      message: 'Getting API credentials...',
      progress: 20
    }));
...
Return ONLY the JSON object following the schema above.`
    ...
    console.error('Error in scan-order-stream function:', error);
    socket.send(JSON.stringify({
      type: 'error',
      message: error instanceof Error ? error.message : 'Unknown error occurred'
    }));
